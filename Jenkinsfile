                                    ...
                                    withCredentials([file(credentialsId: 'docker-credentials', variable: 'DOCKER_CONFIG_JSON')]) {
                                        withEnv(['PATH+EXTRA=/busybox']) {
                                            sh '''#!/busybox/sh
                                                ...
                                                /kaniko/executor --context `pwd` --no-push --digest-file=/shared-data/termination-log --build-arg CI_ENV=Jenkins --build-arg GIT_COMMIT=$GIT_COMMIT --build-arg GIT_BRANCH=$GIT_BRANCH --build-arg VERSION=$VERSION --cache=true --cache-repo=$IMAGE_NAMESPACE/$IMAGE_NAME/cache --target=build-stage --skip-unused-stages
                                                /kaniko/executor --context `pwd` --destination "$IMAGE_NAMESPACE/$IMAGE_NAME:latest" --digest-file=/shared-data/termination-log --build-arg CI_ENV=Jenkins --build-arg GIT_COMMIT=$GIT_COMMIT --build-arg GIT_BRANCH=$GIT_BRANCH --build-arg VERSION=$VERSION --cache=true --target=artifact-stage --cache-repo=$IMAGE_NAMESPACE/$IMAGE_NAME/cache --skip-unused-stages
                                            '''
                                        }
                                    }
                                }
